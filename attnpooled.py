"""
This represents the attention-pooled example, where
a sentence will be encoded into a attention-pooled
...
attention_pool = AttentionPool().to(device)
attention_pool.eval()
...
def get_model_attn_pool(unbatched_sentences, batch_size=64, print_on=True):
    ...
    for sentencesid, sentences in enumerate(batched_sentences):
        ...
        with torch.no_grad():
            pooled_mean = attention_pool.forward(outputs, attention_mask)
...
if runtype == "-c": #create directional vector
    ...
    gptmeanpool = get_model_attn_pool(gpts)

    print("Computing Jacobs...")
    jacobmeanpool = get_model_attn_pool(jacobs)
...
if runtype == "-t": #train decoder model
    ...
    for i in range(2): 
        ...
        for batchno, val in enumerate(ptrange):
            ...
            samples = []
            with torch.no_grad():
                samples = attention_pool.forward(ptoutputs, ptmasks)
    ...
    for sentence, mask in zip(torch.split(jacoboutputs, 1, dim=0), torch.split(jacobmasks, 1, dim=0)):
        with torch.no_grad():
            samples.append(attention_pool.forward(sentence, mask))
    ...
    torch.save(decoder, 'attention_decode.pt')
    torch.save(attention_pool, 'attn_pool.pt')
